	...
	public sealed partial class TileWorld
	{
		...
		public class TileRenderer : MonoBehaviour
		{
			...
			private void OnEnable()
			{
				...
				layer.OnClearLayer += OnClearActiveLayer;
			...
			private void InstantiateTiles(TileLayer layer, GridRect rect)
			{
				...
				for (var i = 0; i < tileCount; i++)
				{
					...
					if (m_ActiveObjects.ContainsKey(coord) == false)
					{
						...
						var go = Instantiate(prefab, worldPos, RotationFromTileFlags(tile), transform);
						go.transform.localScale = ScaleFromTileFlags(tile);
						...
						if (tile.Flags != TileFlags.None)
							Debug.Log($"Instantiate tile: {tile}");
					}
				}
			}

			private Vector3 ScaleFromTileFlags(Tile tile)
			{
			...
			private Quaternion RotationFromTileFlags(Tile tile)
			{
				var flags = tile.Flags;

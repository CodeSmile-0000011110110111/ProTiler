...
{
    /// <summary>
    ///     MasterObjectPooler manages various ObjectPools. By using a MasterObjectPooler, you can perform a large variety of
    ///     pool operations with a named string reference
    ///     instead of requiring an object reference to the ObjectPool. Furthermore, initialization of the pools is handled by
    ///     the MOP.
	...
	{
		[Tooltip(
			"Forces the MOP into singleton mode. This means the MOP will be made scene persistent and will not be destroyed when new scenes are loaded.")]
		[SerializeField] private bool _singletonMode;
		[SerializeField] private ObjectPool[] _pools = new ObjectPool[0];

		private readonly Dictionary<string, ObjectPool> _poolTable = new();
			...
			if (_singletonMode)
			{
				if (Instance == null)
				{
					...
					if (transform.parent == null)
						...
						Debug.LogWarning(
							$"Singleton mode enabled for the Master Object Pooler '{name}' which is not a root GameObject; this means it cannot be made scene persistent");
				}
				else
        ...
        /// </summary>
        /// <param name="poolName">
        ///     Override for the named string reference to use for this pool. By default uses the ObjectPool's
        ///     name.
        /// </param>
        /// <param name="pool">The ObjectPool to add to the MasterObjectPooler.</param>
		...
		{
			foreach (var pool in _poolTable.Values)
        ...
        public GameObject GetObject(string poolName) => GetPool(poolName).GetObject();
        ...
        /// <returns>The retrieved object.</returns>
        public GameObject GetObject(string poolName, Vector3 position) => GetPool(poolName).GetObject(position);
        ...
        /// <returns>The retrieved object.</returns>
        public GameObject GetObject(string poolName, Vector3 position, Quaternion rotation) => GetPool(poolName).GetObject(position, rotation);

        /// <summary>
        ///     Gets an object from the specified pool, and then retrieves the specified component using a cache to improve
        ///     performance.
        ///     Note: this should not be used if multiple components of the same type exist on the object, or if the component will
        ///     be dynamically removed/added at runtime.
        /// </summary>
        ...
        public T GetObjectComponent<T>(string poolName) where T : class => GetPool(poolName).GetObjectComponent<T>();
        ...
        /// <param name="position">The position to set the object to.</param>
        /// <returns>The retrieved component.</returns>
        public T GetObjectComponent<T>(string poolName, Vector3 position) where T : class => GetPool(poolName).GetObjectComponent<T>(position);
        ...
        public T GetObjectComponent<T>(string poolName, Vector3 position, Quaternion rotation) where T : class =>
			GetPool(poolName).GetObjectComponent<T>(position, rotation);
        ...
        public void Release(GameObject obj, string poolName) => GetPool(poolName).Release(obj);

        /// <summary>
        ///     Releases a collection of objects and returns them back to the specified pool, effectively 'destroying' them from
        ///     the scene.
        /// </summary>
        /// <param name="objs">the objects to release.</param>
        /// <param name="poolName">The name of the pool to return the objects to.</param>
        public void Release(IEnumerable<GameObject> objs, string poolName) => GetPool(poolName).Release(objs);

        /// <summary>
        ///     Releases every active object in the specified pool.
        /// </summary>
        /// <param name="poolName">The name of the pool.</param>
        public void ReleaseAll(string poolName) => GetPool(poolName).ReleaseAll();
        ...
        /// </summary>
        /// <param name="poolName">The name of the pool to populate.</param>
        public void Purge(string poolName) => GetPool(poolName).Purge();
        ...
        public IEnumerable<GameObject> GetAllActiveObjects(string poolName) => GetPool(poolName).GetAllActiveObjects();
	}
}
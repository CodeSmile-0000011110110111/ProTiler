	...
	public unsafe struct NativeArray2D<T> : IDisposable, IEnumerable<T>, IEquatable<NativeArray2D<T>> where T : unmanaged
	{
		...
		public T this[int index0, int index1]
		{
			get
			{
				...
				var index = index1 * m_Length0 + index0;
...
// Make sure we're not double-disposing
		...
		private static void Copy(NativeArray2D<T> src, NativeArray2D<T> dest)
		{
			...
			for (var index0 = 0; index0 < src.Length0; ++index0)
			{
				for (var index1 = 0; index1 < src.Length1; ++index1)
		...
		private static void Copy(T[,] src, NativeArray2D<T> dest)
		{
			...
			for (var index0 = 0; index0 < dest.Length0; ++index0)
			{
				for (var index1 = 0; index1 < dest.Length1; ++index1)
					dest[index0, index1] = src[index0, index1];
			}
		}
	...
	internal sealed class NativeArray2DDebugView<T> where T : unmanaged
	{
		...
		///     The array to view
		/// </param>
		public NativeArray2DDebugView(NativeArray2D<T> array) => m_Array = array;
	}
}

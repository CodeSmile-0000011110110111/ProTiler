...
[assembly: InternalsVisibleTo("QFSW.MOP2.Editor")]

namespace QFSW.MOP2
{
    /// <summary>
    ///     Object pool containing several copies of a template object (usually a prefab). Using the pool with GetObject and
    ///     Release
	...
	public class ObjectPool : ScriptableObject
	{
		...
		[Tooltip(
			"The maximum number of objects that can be kept in this pool. If it is exceeded, objects will be destroyed instead of pooled when returned. Set to -1 for no limit.")]
		...
		[SerializeField] private bool _repopulateOnSceneChange;
		private readonly Regex _poolRegex = new("[_ ]*[Pp]ool");
		private readonly List<GameObject> _pooledObjects = new();
		private readonly Dictionary<int, GameObject> _aliveObjects = new();

		private readonly List<GameObject> _releaseAllBuffer = new();
		private readonly Dictionary<(int id, Type type), object> _componentCache = new();
        ...
        /// </summary>
        /// <param name="template">
        ///     The template object to center the pool on. All objects in the pool will be a copy of this
        ///     object.
        /// </param>
        /// <param name="defaultSize">The default number of objects to create in this pool when initializing it.</param>
        /// <param name="maxSize">
        ///     The maximum number of objects that can be kept in this pool. If it is exceeded, objects will be
        ///     destroyed instead of pooled when returned. Set to -1 for no limit.
        /// </param>
        ...
        public static ObjectPool Create(GameObject template, int defaultSize = 0, int maxSize = -1) =>
			Create(template, template.name, defaultSize, maxSize);
		...
		{
			var pool = Create(template, defaultSize, maxSize);
			pool.Initialize();
		...
		{
			var pool = Create(template, name, defaultSize, maxSize);
			pool.Initialize();
        ...
        /// <summary>
        ///     If enabled, object instances will be renamed to ObjectName#XXX where XXX is the instance number. This is useful if
        ///     you want them all to be uniquely named.
        /// </summary>
        public bool IncrementalInstanceNames { get => _incrementalInstanceNames; set => _incrementalInstanceNames = value; }
		...
		{
			foreach (var poolable in _template.GetComponentsInChildren<IPoolable>())
				poolable.InitializeTemplate(this);
		}

		private GameObject CreateNewObject() => CreateNewObject(_template.transform.position, _template.transform.rotation);

		private GameObject CreateNewObject(Vector3 position, Quaternion rotation)
		{
			var newObj = Instantiate(_template, position, rotation);
        ...
        public GameObject GetObject() => GetObject(_template.transform.position);
        ...
        /// <returns>The retrieved object.</returns>
        public GameObject GetObject(Vector3 position) => GetObject(position, _template.transform.rotation);
        ...
        public T GetObjectComponent<T>() where T : class => GetObjectComponent<T>(_template.transform.position);
        ...
        /// <param name="position">The position to set the object to.</param>
        /// <returns>The retrieved component.</returns>
        public T GetObjectComponent<T>(Vector3 position) where T : class => GetObjectComponent<T>(position, _template.transform.rotation);
		...
		{
			var obj = GetObject(position, rotation);
        ...
        public void Populate(int quantity, PopulateMethod method = PopulateMethod.Set)
		{
			...
			switch (method)
			{
				case PopulateMethod.Set:
					newObjCount = quantity - _pooledObjects.Count;
					break;
				case PopulateMethod.Add:
					newObjCount = quantity;
					break;
				default:
					newObjCount = 0;
					break;
			...
			if (HasMaxSize)
				newObjCount = Mathf.Min(newObjCount, _maxSize - _pooledObjects.Count);
			if (newObjCount < 0)
				newObjCount = 0;

			for (var i = 0; i < newObjCount; i++)
			{
        ...
        public void Purge()
		{
			foreach (var obj in _pooledObjects)
				Object.Destroy(obj);
			foreach (var obj in _aliveObjects.Values)
				Object.Destroy(obj);
			_pooledObjects.Clear();
			_aliveObjects.Clear();
			_componentCache.Clear();
        ...
        public IEnumerable<GameObject> GetAllActiveObjects() => _aliveObjects.Values
			.Where(x => x);

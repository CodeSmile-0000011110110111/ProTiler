...
{
	internal class TileDataProxyMoveTool : EditorTool
	{
		private Vector3 m_Origin;
		private Vector3 m_Current;

		private void OnEnable() => Debug.Log($"{GetType()} OnEnable");

		private void OnDisable() => Debug.Log($"{GetType()} OnDisable");
		...
		public override void OnToolGUI(EditorWindow window)
		{
			...
			if (evType == EventType.MouseDown && control != GUIUtility.hotControl)
				StartMove(UnityEditor.Tools.handlePosition);

			var pos = Handles.PositionHandle(UnityEditor.Tools.handlePosition, UnityEditor.Tools.handleRotation);
			...
			if (EditorGUI.EndChangeCheck())
			{
				...
				Debug.Log($"handle: {UnityEditor.Tools.handlePosition}, new: {pos}, diff: {diff} - {target} {target.GetType()}");
